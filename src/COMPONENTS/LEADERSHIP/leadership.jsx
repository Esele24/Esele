import styles from "./leadership.module.css"
import { Mail } from "lucide-react"
import { Instagram } from "lucide-react"
import { Phone } from "lucide-react"
import Link from "next/link"
export function Leadership({}){
    return (
        <div>
            <div className={styles.leadership} id="leadership-section">
                <div className={styles.begin}>
                    <br></br>
                    <div className={styles.top}>
                        <label className={styles.txt}>
                            <b>Our</b>
                        </label>
                            <br></br>
                        <label className={styles.fell}>
                            <b>Leadership</b>
                        </label>
                    </div>
                </div>
                <div className={styles.bottom}>
                    <div className={styles.mission}>
                        <label className={styles.text}>
                            Meet the dedicated students who serve our fellowship community with passion and
                        </label>
                        <br></br>
                        <label className={styles.mid}>
                            commitment. They're here to support you on your spiritual journey and help you find
                        </label>
                        <label className={styles.end}>
                            your place in our community.
                        </label>
                    </div>
                </div>
                <div className={styles.leaders}>
                    <div className={styles.stack}>
                        <div className={styles.circle}>
                            <img className={styles.esele} src="/IMG-20250312-WA0069.jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.poin}>Esele E. Okogbo</label>
                            <label className={styles.sele}>President</label>
                            <label className={styles.seles}><center>B.Sc Software Engineering (in view)</center></label>
                            <br></br>
                            <label className={styles.point}>Passionate about creating inclusive</label>
                            <label className={styles.point}>spaces for spiritual growth and</label>
                            <label className={styles.point}>community building</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202300112@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/eseleokogbo">eseleokogbo</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>09036669156</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                    <div className={styles.stack}>
                        <div className={styles.square}>
                            <img className={styles.pre} src="IMG-20240611-WA0031.jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.poin}>Precious N. Onah</label>
                            {/* <br></br> */}
                            <label className={styles.pele}>Vice President</label>
                            {/* <br></br> */}
                            <label className={styles.peles}>LL.B Law (in view)</label>
                            <br></br>
                            <br></br>
                            <label className={styles.point}>Dedicated to helping students</label>
                            <label className={styles.point}>discover their purpose through faith</label>
                            <label className={styles.point}>and fellowship.</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202100062@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/omo_baba17">omo_baba17</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>08142302738</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                    <div className={styles.stack}>
                        <div className={styles.square}>
                            <img className={styles.pre} src="IMG-20250801-WA0181.jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.poin}>Godson O. Jacob</label>
                            {/* <br></br> */}
                            <label className={styles.gele}>Secretary General</label>
                            <br></br>
                            <label className={styles.peles}>LL.B Law (in view)</label>
                            <br></br>
                            <br></br>
                            <label className={styles.point}>Dedicated to grooming sstudents'</label>
                            <label className={styles.point}>spiritual life through discipleship, mentorship</label>
                            <label className={styles.point}>and fellowship.</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202400323@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/godson10011">godson10011</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>08062169811</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                </div>
                <div className={styles.leaders}>
                    <div className={styles.stack}>
                        <div className={styles.circle}>
                            <img className={styles.esele} src="IMG-20250801-WA0174.jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.poinn}>Emmanuel C. Egwu</label>
                            <br></br>
                            <label className={styles.cele}>Deputy Secratary General</label>
                            <label className={styles.seles}>LL.B Law (in view)</label>
                            <br></br>
                            <label className={styles.point}>Passionate about souls,prayer,worship</label>
                            <label className={styles.point}>word study, and creating an enabling</label>
                            <label className={styles.point}>environment for peopele to grow in the faith</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202300062@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/chibuikem_egwu">chibuikem_egwu</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>08166636128</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                    <div className={styles.stack}>
                        <div className={styles.square}>
                            <img className={styles.pre} src="IMG-20250803-WA0154(1).jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.pointt}>Precious I. Michael</label>
                            <br></br>
                            <label className={styles.peele}>Treasurer</label>
                            <br></br>
                            <label className={styles.pelles}>LL.B Law (in view)</label>
                            <br></br>
                            <br></br>
                            <label className={styles.point}>Keeping financial records</label>
                            <label className={styles.point}>safe, secure and </label>
                            <label className={styles.point}>fostering accountability</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202200098@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/michaelprecious727">michaelprecious727</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>07065882434</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                    <div className={styles.stack}>
                        <div className={styles.square}>
                            <img className={styles.pre} src="IMG-20250819-WA0250.jpg"/> 
                        </div> 
                        <br></br>
                        <div className={styles.faith}>
                            <label className={styles.poin}>Success G. Patrick</label>
                            {/* <br></br> */}
                            <label className={styles.geles}>Public Relations Officer</label>
                            <br></br>
                            <label className={styles.peles}>LL.B Law (in view)</label>
                            <br></br>
                            <br></br>
                            <label className={styles.point}>Ensuring healthy PR between fellowship, its</label>
                            <label className={styles.point}>members, the school, friends</label>
                            <label className={styles.point}>families, and thw rold at large</label>
                            {/* <br></br> */}
                            <div className={styles.mal}>                                
                                <label className={styles.pop}>202200179@topfaith.edu.ng</label>
                                <Mail fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.mall}>                                
                                <label className={styles.popp}><Link className={styles.link} href="https://instagram.com/_s.u.x.y_">_s.u.x.y_</Link></label>
                                <Instagram fill="none" color="navy" size={35}/>
                            </div>
                            <div className={styles.malll}>                                
                                <label className={styles.pop}>07050377172</label>
                                <Phone fill="none" color="navy" size={35}/>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}